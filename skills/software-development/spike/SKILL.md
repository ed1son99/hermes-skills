...
triggers: [spike, prototype, experiment, feasibility, throwaway, exploration, research, planning, mvp, proof-of-concept]
...
## Single-file HTML prototyping pitfalls

When building interactive HTML/Canvas prototypes as spikes, watch for these failure modes (detailed in `references/html-canvas-pitfalls.md`):

1. **Variable/function name conflicts crash silently.** `let project = {...}` followed by `function project() {...}` causes `Identifier has already been declared` and kills all JS — the page renders static HTML with zero errors in DevTools but nothing works. Always run `node -e "new Function(scriptContent)"` before opening the file.

2. **Patch-stacking destroys scope.** Three or more successive `patch` tool calls on a single HTML file easily leave functions accidentally nested inside other functions (indentation drifts by 2 spaces). When a patch that was supposed to add global functions instead adds them inside `getCanvas()` or `setupCanvas()`, variables become inaccessible from the console and event handlers silently fail. If you've patched the same file 3+ times and are chasing invisible bugs, **stop patching and rewrite the entire file with `write_file`.**

3. **Canvas covers interactive elements.** A full-size `<canvas>` with `cursor:grab` inside a `position:relative` container does NOT capture clicks on absolutely-positioned sibling elements (`z-index:10` wins). But if the canvas has `pointer-events: all` set explicitly or the toolbar lacks `position:absolute`, buttons become unclickable. Always position toolbar/minimap with `position:absolute; z-index:10` inside the canvas wrapper.

4. **file:// protocol restrictions.** Some browsers (notably Safari) block certain JS APIs from `file://` origins. Chrome usually allows everything. If the page works in one browser but not another, test with a local HTTP server: `python3 -m http.server 8080`.
