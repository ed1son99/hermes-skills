...
**JavaScript silent script failures:** When a `<script>` block has a parse-time error (e.g., `let x` followed by `function x()`), the browser aborts the entire script silently. The page renders static HTML, all DOM elements exist, DevTools may show zero errors or empty `js_errors` entries, but no JavaScript runs. Detection: run `node -e "new Function(scriptContent)"` to catch the syntax error. Common cause: variable/function name collisions in the same scope.
